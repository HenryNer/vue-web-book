    ...
    export default {
        ...
        mounted() {
            this.task = setInterval(() => {
                this.$refs.mask.forEach((mask, index) => {
                    ...
                    if (index === 0) {
                        ...
                        if (this.add && maskWidth.value < 16) {
                            let preMaskWidth = this.maskWidth[index - 1].value
                            if (preMaskWidth >= 8) {
                                maskWidth.value++
                                lineWidth.value--
                            }
                        } else if (!this.add && lineWidth.value < 16) {
                            let preLineWidth = this.lineWidth[index - 1].value
                            if (preLineWidth >= 8) {
                                maskWidth.value--
                                lineWidth.value++
                            }
                    ...
                    if (index === this.maskWidth.length - 1) {
                        if (maskWidth.value === 16 && this.maskWidth[0].value === 16) {
                            this.add = !this.add
                        } else if (maskWidth.value === 0 && this.maskWidth[0].value === 0) {
                            this.add = !this.add
                        }
                    }
                })
