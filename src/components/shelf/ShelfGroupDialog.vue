...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      moveToGroup(group) {
        const ifRepeat = this.shelfSelected.every(book => group.itemList.indexOf(book) !== -1)
        if (ifRepeat) {
          ...
          this.setShelfList(this.shelfList
            .filter(book => {
              ...
              //确认把选中的电子书在需要移动的分组中没有重复
              return this.shelfSelected.indexOf(book) < 0
              }))
            .then(() => {
              //把选中的图书加到目标分类组中
              if (group && group.itemList) { //group为移动到功能的每个选项
